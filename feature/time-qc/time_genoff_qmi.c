...
int time_genoff_operation(time_genoff_info_type *pargs)
{
	...
	if (pargs->operation == T_GET) {
		TIME_LOGE("Lib:%s: pargs->base = %d\n", __func__, pargs->base);
		TIME_LOGE("Lib:%s: pargs->operation = %d\n", __func__,pargs->operation);
	} else {
		TIME_LOGE("Lib:%s: pargs->base = %d\n", __func__, pargs->base);
		TIME_LOGE("Lib:%s: pargs->operation = %d\n", __func__,pargs->operation);
	}
	...
	if (pargs->operation == T_GET) {
		TIME_LOGE("Lib:%s: pargs->ts_val = %llu\n", __func__,
	...
	TIME_LOGE("sun_path is %s\n",time_socket.sun_path);
    if (!getcwd(cwd, sizeof(cwd)))
	{
    TIME_LOGE("1111 getcwd cwd is %s\n",cwd);
	}
    TIME_LOGE("2222 getcwd cwd is %s\n",cwd);
	...
	if (connect(sock_id, (struct sockaddr *)&time_socket, length) == -1) {
		TIME_LOGE("errno is %d\n,sock_id is %d,socket name is %s,socket name is %s\n",errno,sock_id,GENOFF_SOCKET_NAME,time_socket.sun_path);
	...
	if (pargs->operation == T_GET) {
		...
		if (send(sock_id, &to_send, sizeof(to_send), 0) < 0) {
			...
			TIME_LOGE("Lib:%s: Send to server  passed pid %d\n",
					__func__, getpid());
			if (recv(sock_id, (void *)&to_send, sizeof(to_send), 0) < 0) {
				TIME_LOGE("Lib:%s: Unable to recv data\n",__func__);
				goto error_close_socket;
			}
			TIME_LOGE("Lib:Receive Passed == base = %d, unit = %d,"
		...
		if (send(sock_id, &to_send, sizeof(to_send), 0) < 0) {
			...
			TIME_LOGE("Lib:%s: Send to server  passed!!\n", __func__);
			if (recv(sock_id, (void *)&to_send, sizeof(to_send), 0) < 0) {
				TIME_LOGE("Lib:%s: Unable to recv data\n",__func__);
				goto error_close_socket;
			}
