...
#if !ADB_HOST
int reassgin_usb_serial(void)
{
	char *usb_serial_tag = "sp-";
	const char *usb_serial_num_file[2] = {
		"/data/usb_serial_num.txt",
		"/data/usb_serial_num_random.txt",
	};
	const char *usb_serial_path[3] = {
		"/sys/bus/platform/drivers/android_usb/serialnumber",
		"/sys/devices/virtual/android_usb/android0/iSerial",
		NULL,
	};
	FILE *fd_serial;
	int usb_serial_generate_random;
	int ret, i = 0;
	char usb_serial_num[128];
	char *p = usb_serial_num;
	char *p_end = usb_serial_num + sizeof usb_serial_num;

	// PROP_NAME_MAX is 32 bytes till now
	property_get("persist.adb.usb.serial.random", usb_serial_num, "0");
	usb_serial_generate_random = strtol(usb_serial_num, NULL, 0);
	memset(usb_serial_num, 0, sizeof usb_serial_num);
	fd_serial = fopen(usb_serial_num_file[usb_serial_generate_random ? 1:0], "a+");

	fseek(fd_serial, 0L, SEEK_SET); // android bionic fopen mode "a+" has bug, must seek to 0 manually
	if (usb_serial_generate_random)
		p += sprintf(p, "%s", usb_serial_tag);
	ret = fscanf(fd_serial, "%s", p);
	if (p[0] == 0 && usb_serial_generate_random) {
		int fd_urandom = unix_open("/dev/urandom", O_RDWR);
		ret = 1;
		D("[ create random serial from /dev/urandom start... ]\n");
		if (fd_urandom >= 0) {
			int tmp_usb_serial;
			struct timeval tv;
			gettimeofday(&tv, NULL);
			unix_write(fd_urandom, &tv, sizeof tv);
			unix_read(fd_urandom, &tmp_usb_serial, sizeof tmp_usb_serial);
			fwrite(p, 1, sprintf(p, "%08x", tmp_usb_serial), fd_serial);
			unix_close(fd_urandom);
			D("[ create random serial from /dev/urandom success ]\n");
		}
	} else ret = 0;

	while (usb_serial_path[i]) {
		if (access(usb_serial_path[i], R_OK | W_OK) == 0)
			break;
		i++;
	}

	if (usb_serial_path[i]) {
		int fd_usb_serial;
		fd_usb_serial =	unix_open(usb_serial_path[i], O_RDWR);
		if (fd_usb_serial >= 0) {
			if (p[0])
				unix_write(fd_usb_serial, usb_serial_num, strlen(usb_serial_num));
			else {
				unix_read(fd_usb_serial, p, p_end - p - 1);
				fwrite(p, 1, strlen(p), fd_serial);
			}
		}
		unix_close(fd_usb_serial);
		D("[ reassgin adb usb serial value to %s ]\n", usb_serial_num);
	} else D("[ can't detect adb usb serial file %s ]\n", usb_serial_num);
