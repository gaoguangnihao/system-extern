...
def BuildVerityTree(sparse_image_path, verity_image_path, prop_dict):
  ...
  print "FIXED_SALT is %s" % FIXED_SALT
  return True
...
def MakeVerityEnabledImage(out_file, prop_dict):
  ...
  shutil.copy2(out_file,"/home/dhcui/system-org.img")
...
def BuildImage(in_dir, prop_dict, out_file, target_out=None):
  ...
  ext4fs_output = None
  print "build_command is %s" % build_command
  try:
  ...
  # sprout).
  print "build system image reserved_blocks"
  if reserved_blocks and fs_type.startswith("ext4"):
    ...
    if used_blocks > adjusted_blocks:
      ...
      print("Error: Not enough room on %s (total: %d blocks, used: %d blocks, "
            "reserved: %d blocks, available: %d blocks)" % (
                ...
                adjusted_blocks))
      return False
  print "build system image fs_spans_partition value is %d" % fs_spans_partition
  ...
  print "build system image verity_supported value is %d" % verity_supported
  print "build system image is_verity_partition value is %d " % is_verity_partition
  print "build system image MakeVerityEnabledImage is %s" % prop_dict
  if verity_supported and is_verity_partition:
...
def main(argv):
  ...
  print "dhcui-debug 1111111111111111111111"
  print "in_dir is %s" % in_dir
  print "image_properties %s" % image_properties
  print "out_file %s" % image_properties
  print "target_out %s" % target_out
